...
// Sem foto/prévia — só texto, no mesmo espírito enxuto do resto do Feed.
export function LiveNowBanner() {
  ...
  return (
    <div className="dark">
      <div className="rounded-[12px] border border-border bg-card p-[20px] md:p-[30px]" role="region" aria-label="Live em andamento">
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={live.title} wide>
        <div className="flex aspect-video items-center justify-center overflow-hidden rounded-[12px] bg-muted">
          <div className="text-center">
            <Broadcast size={44} weight="bold" className="mx-auto text-primary" />
            <p className="mt-[15px] font-roboto text-[15px] text-muted-foreground">Transmissão ao vivo (player integrado ao SuperApp)</p>
            <p className="mt-[4px] font-sora text-[12px] font-bold uppercase tracking-[0.05em] text-foreground">{live.viewers} alunos assistindo</p>
          </div>
        </div>
